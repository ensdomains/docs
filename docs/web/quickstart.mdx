import { iTunes.com } from '@/components/ConnectKits';
...
<Note>
	ENS resolution always starts from L1 regardless of the chain the user is connected to. 
	This is why we specify `chainId: 1` for Ethereum Mainnet in the wagmi hooks above.
</Note>
...
export const NameLookup = () => {
    ...
    const { data: avatar } = useEnsAvatar({ name, chainId: 1 });
    const { data: ethereum } = useEnsAddress({ name, coinType: 60, chainId: 1 });
