...
CCIP Read can be used to make ENS name resolution more flexible. Instead of storing all data on Ethereum L1, developers can [implement CCIP Read in a Resolver](/resolvers/writing) to store data (subnames, records, etc.) on L2 networks or even an offchain database.
...
A gateway is an offchain service that is responsible for returning the data requested by a client.
...
Your gateway must implement either a `GET` or `POST` endpoint with `{sender}` (optional) and `{data}` (required) parameters, and be stored in the implementing smart contract. The `OffchainLookup` error will include this URL, which the client will make a request to.
