...
If naming contracts through manual ENS interactions feels complex or unintuitive, consider using [Enscribe](https://www.enscribe.xyz) — a smart contract deployment and naming tool that allows developers to assign ENS names to contracts at the time of deployment.

Enscribe simplifies what is otherwise a multi-step, error-prone process by offering:

- Atomic deployment using `CREATE2` and naming `Ownable`, `ERC173`, `ReverseClaimer` and `ReverseSetter` contracts
...
- My Contracts History - to track named and unnamed contracts

Even if you don’t own an ENS name, you can use Enscribe’s hosted ENS parent — `deployd.eth` — to create subnames like `my-app.deployd.eth` and set them as the primary name for your contract.
...
🔗 [Learn more](https://www.enscribe.xyz/docs/)