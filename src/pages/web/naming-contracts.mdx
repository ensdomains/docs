...
The simplest way to achieve this is using the [Enscribe app](https://app.enscribe.xyz/).
...
Enscribe has been created to simplify naming smart contracts with ENS names. Its [app](https://app.enscribe.xyz/) allows users to deploy smart contracts with primary ENS names set as well as easily locate and name their existing smart contracts.

Enscribe simplifies what is otherwise a multi-step, error-prone process by offering:

- Atomic deployment using `CREATE2` and naming `Ownable`, `ERC173`, `ReverseClaimer` and `ReverseSetter` contracts described above
- ENS subname creation, forward resolution and Primary name (reverse record) assignment
- Post-deployment naming of existing contracts
- Contracts history - to easily locate contracts that can be named and those you have already named

Additionally, even if you don’t own an ENS name, you can use Enscribe’s hosted ENS parent — `deployd.eth` — to create subnames like `my-app.deployd.eth` and set them as the primary name for your contract.

Enscribe is fully on-chain, stateless, and supports the following networks:
- **Mainnets**: Ethereum, Linea, Base
- **Testnets**: Sepolia, Linea Sepolia, Base Sepolia
