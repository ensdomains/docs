...
While forward resolution is configured in [Resolvers](/resolvers/quickstart), reverse records are managed by smart contracts called Reverse Registrars which you can [read more about below](#setting-primary-names).
...
ENS names can resolve to [different addresses on different chains](/web/resolution), and since `nick.eth` in the example above has only specified an Ethereum Mainnet address, the verification process will fail. In order to fix this, I need to set the Base address for `nick.eth` which is on L1 in this case. This is done by calling `setAddr(namehash("nick.eth"), 0x1234...5678)` on the resolver for the name.
...
:::warning
As of August 12, 2025, no library has been updated to support L2 Primary Names. Refer to [this temporary reference implementation](https://github.com/ensdomains/frontend-template/blob/main/src/hooks/usePrimaryName.ts).
:::
