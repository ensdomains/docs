...
Even if your application only operates on an L2 like Base, ENS resolution always starts on Ethereum Mainnet. This means you need to configure a Mainnet client alongside your L2 chain.
...
```tsx [Wagmi]
import { createConfig, http, useEnsAddress, toCoinType } from 'wagmi'
...
### Chain-Specific Addresses and Default Address

It is possible to configure a different address per chain for the same name:
- `test.ses.eth` resolves to `0x2B0F09F23193de2Fb66258a10886B9f06903276c` on Ethereum Mainnet, but
- `test.ses.eth` resolves to `0x7d3a48269416507E6d207a9449E7800971823Ffa` on Base.

From an application point of view it is important to be aware and always request the address for the correct chain, even on Ethereum Mainnet. All examples above explicitly set the `coinType` to Base, since they request the address on Base for a given name.