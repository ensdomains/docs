...
For root-level (`ROOT_RESOURCE`) permissions, you can also use `grantRootRoles()` / `revokeRootRoles()` inherited from [EAC](/contracts/ensv2/enhanced-access-control). `grantRoles()` and `revokeRoles()` are disabled on the Permissioned Resolver; use the `authorize*` functions for name and record-level permissions.
</FrenCallout>
...
Each account deploys its own resolver instance. See [Deploying a Resolver Proxy](/contracts/ensv2/verifiable-factory#deploying-a-resolver-proxy) for a code example showing how to deploy a Permissioned Resolver through the Verifiable Factory. The examples below assume you already have a `resolverAddress`.
