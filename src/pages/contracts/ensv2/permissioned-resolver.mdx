...
Note that aliasing at the resolver level is different from [namespace aliasing](/contracts/ensv2/registry-hierarchy#namespace-aliasing) at the registry level. Resolver aliasing shares records; registry aliasing shares entire namespaces.
...
Each account deploys its own resolver instance. See [Deploying a Proxy](/contracts/ensv2/verifiable-factory#deploying-a-proxy) for a code example showing how to deploy a Permissioned Resolver through the Verifiable Factory. The examples below assume you already have a `resolverAddress`.
