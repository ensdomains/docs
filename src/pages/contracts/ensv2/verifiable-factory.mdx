...
Deploy a [Permissioned Resolver](/contracts/ensv2/permissioned-resolver) proxy through the factory. The same pattern applies to any implementation (e.g., `UserRegistryImpl` for subname registries): encode the implementation's `initialize` call as the `data` argument to `deployProxy`.

<FrenCallout fren="bittu" variant="tip">
The `VERIFIABLE_FACTORY` and `PERMISSIONED_RESOLVER_IMPL` addresses used below are protocol-deployed contracts. Find them in the [Deployments](/contracts/ensv2/overview#deployments-sepolia) table.
</FrenCallout>
...
// Unique salt per deployment (reusing a salt from the same sender will revert)
const salt = BigInt(keccak256(stringToBytes('my-resolver-v1')))

// Encode the implementation's initializer as calldata
...
// Parse the ProxyDeployed event to get the new address
