...
ENSv2's DNS resolution uses off-chain DNSSEC verification via [CCIP-Read](/resolvers/ccip-read). The on-chain DNSSEC proof submission used by the v1 [DNS Registrar](/registry/dns) for claiming DNS names remains on v1 infrastructure at launch.
</FrenCallout>
...
This causes `example.com` to resolve as `alice.eth`. Subdomains also resolve as `alice.eth` if a DNS wildcard record is configured (e.g., `*.example.com` with the same TXT record). Useful when you want a DNS domain to mirror an existing ENS name.
