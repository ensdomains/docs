...
Resolvers are separate contracts ([Permissioned Resolver](/contracts/ensv2/permissioned-resolver)) deployed per-account as UUPS proxies. All names owned by the same account share one resolver. Names can also share records across resolvers via [aliases](/contracts/ensv2/permissioned-resolver#aliasing) (`setAlias`), which rewrite the name suffix during resolution (e.g., `sub.alias.eth` → `sub.test.eth`).
...
:::note
A `RegistryCreated()` event (emitted by PermissionedRegistry in the constructor, and by UserRegistry/WrapperRegistry on `initialize()`) is being added in [contracts-v2#258](https://github.com/ensdomains/contracts-v2/pull/258). This will allow indexers to discover new registry deployments on-chain.
:::
