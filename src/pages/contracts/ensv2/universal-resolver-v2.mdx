...
2. **Locked canonical parent**: on the subregistry, call `hasAssignees(ROOT_RESOURCE, ROLE_SET_PARENT | ROLE_SET_PARENT_ADMIN)`. This must return `false`. Otherwise the subregistry can be [remounted](/contracts/ensv2/permissioned-registry#parent-pointer) at a different position (e.g. moved from `nick.eth` to `nick.xyz`), changing what name the tokens represent.
