...
The registry also stores its own parent via `setParent()` / `getParent()`, which records both the parent registry address and the child label. This backward pointer is used by the Universal Resolver's [`findCanonicalName()`](/contracts/ensv2/universal-resolver-v2#registry-navigation) to verify hierarchy integrity.

A registry has a **canonical parent** once `setParent(parent, label)` has been called, establishing where it sits in the hierarchy. A registry has a **locked canonical parent** when the owner additionally revokes `ROLE_SET_PARENT` (and `ROLE_SET_PARENT_ADMIN`) on `ROOT_RESOURCE`, making the backward pointer immutable. Without this, the registry could be remounted at a different position in the hierarchy (e.g. moved from `nick.eth` to `nick.xyz`).
...
<ContractReference functions={[
  {
    ...
    params: [
      ...
      { name: 'registry', type: 'IRegistry', description: 'Child subregistry to set, or address(0)' },
  ...
  {
    name: 'setSubregistry',
    description: 'Set the child registry for a name. Pointing two names to the same registry creates a namespace alias.',
    params: [
      { name: 'anyId', type: 'uint256', description: 'Labelhash, token ID, or resource of the name' },
      { name: 'registry', type: 'IRegistry', description: 'Child registry address' },
    ],
  },
  {
    name: 'setResolver',
...
<ContractReference functions={[
  ...
  {
    name: 'getSubregistry',
    description: 'Get the child registry for a label. Returns address(0) if expired.',
    params: [
...
<ContractReference functions={[
  ...
  },
  {
    ...
    description: 'Child registry changed.',
    params: [
      { name: 'tokenId', type: 'uint256', description: 'Token ID of the name' },
      { name: 'subregistry', type: 'IRegistry', description: 'New child registry address' },
