...
Subname registries are deployed as UserRegistry proxies through the Verifiable Factory. See [Deploying a Proxy](/contracts/ensv2/verifiable-factory#deploying-a-proxy) for a code example. The examples below use the ETHRegistry, but the same functions are available on any PermissionedRegistry instance.
