...
The Universal Resolver's [`findCanonicalName()`](/contracts/ensv2/universal-resolver-v2#registry-navigation) uses these backward pointers to walk up the tree, verifying at each step that `parent.getSubregistry(label)` points back to the current registry, and reconstructing the full name along the way.
...
For a subname like `sub.alice.eth` to be fully emancipated, the `alice.eth` owner must emancipate their registry (revoke the roles listed above on `ROOT_RESOURCE`) and lock the [canonical parent](#parent-pointer) by revoking `ROLE_SET_PARENT` on `ROOT_RESOURCE` (so the registry can't be re-mounted elsewhere in the hierarchy).
...
<ContractReference functions={[
  {
    ...
    params: [
      ...
      { name: 'labelHash', type: 'bytes32', description: 'Hash of the registered label' },
      ...
      { name: 'sender', type: 'address', description: 'Caller that triggered the registration' },
    ],
  },
  {
    name: 'LabelReserved',
    ...
    params: [
      { name: 'tokenId', type: 'uint256', description: 'Token ID of the reserved name' },
      { name: 'labelHash', type: 'bytes32', description: 'Hash of the reserved label' },
      { name: 'label', type: 'string', description: 'The reserved label' },
      { name: 'expiry', type: 'uint64', description: 'Expiry timestamp' },
      { name: 'sender', type: 'address', description: 'Caller that triggered the reservation' },
    ],
  },
  {
    ...
    params: [
      { name: 'tokenId', type: 'uint256', description: 'Token ID of the unregistered name' },
      { name: 'sender', type: 'address', description: 'Caller that triggered unregistration' },
    ],
  },
  {
    name: 'ExpiryUpdated',
    ...
    params: [
      ...
      { name: 'sender', type: 'address', description: 'Caller that triggered the renewal' },
    ],
  },
  {
    ...
    params: [
      { name: 'tokenId', type: 'uint256', description: 'Token ID of the name' },
      { name: 'subregistry', type: 'IRegistry', description: 'New subregistry address' },
      { name: 'sender', type: 'address', description: 'Caller that made the change' },
    ],
  },
  {
    ...
    params: [
      { name: 'tokenId', type: 'uint256', description: 'Token ID of the name' },
      { name: 'resolver', type: 'address', description: 'New resolver address' },
      { name: 'sender', type: 'address', description: 'Caller that made the change' },
    ],
  },
  {
    ...
    params: [
      { name: 'oldTokenId', type: 'uint256', description: 'Previous token ID' },
      { name: 'newTokenId', type: 'uint256', description: 'New token ID' },
    ],
  },
  {
    name: 'ParentUpdated',
    ...
    params: [
      { name: 'parent', type: 'IRegistry', description: 'New parent registry' },
