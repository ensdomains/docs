...
Unlike the standard ERC1155 which has a `_uri` storage variable, ERC1155Singleton makes `uri()` abstract with no built-in storage. The [Permissioned Registry](/contracts/ensv2/permissioned-registry) delegates `uri()` to an external [metadata provider](/contracts/ensv2/registry-metadata) via the `MetadataMixin` pattern.
