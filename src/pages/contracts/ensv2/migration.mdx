...
Migration clears the **resolver** of the migrated name (when possible). For unwrapped names, v1 registry ownership is also transferred to the Graveyard. The remainder of the v1 registry namespace is left unchanged. The Graveyard can separately clean up the unemancipated namespace of any migrated name:
...
Names that *could* migrate but don't will eventually expire. Their v1 tokens expire, leaving the v1 registry state frozen (resolver, owner, fuses all unchanged). Since v1 registrations are disabled, names never become available in v1 again. The v2 RESERVED slot also expires. Once expired in v2, the name becomes available for fresh registration via the `ETHRegistrar`.
