...
Once ENSv2 is live, v1 `.eth` registrations and renewals are disabled. All renewals and new registrations go through the v2 [ETH Registrar](/contracts/ensv2/eth-registrar). Names in v1 will simply expire and can never be re-registered through v1.
</FrenCallout>
...
After premigration, v1 `.eth` registrations and v1 renewals are disabled. All renewals go through the v2 `ETHRegistrar`, which syncs the expiry back to v1 when necessary. Names continue to resolve through v1 during this phase because `ENSV1Resolver` mirrors v1 resolution.
