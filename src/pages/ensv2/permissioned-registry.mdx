...
stateDiagram-v2
    [*] --> AVAILABLE
    AVAILABLE --> REGISTERED: register() or renew()
    AVAILABLE --> RESERVED: register() with owner=0 or renew()
...
- The transfer reverts with `TransferUnsafeUntilRegistryIsEmancipated` if the registry is not [emancipated](#emancipation), meaning an account with dangerous root roles could override the new owner.
...
A registry is **emancipated** when no account holds a dangerous role on its `ROOT_RESOURCE`. Every registry reports this via `isEmancipated()`.
</FrenCallout>

Roles in a registry are normally scoped to a single name, but a role granted on [`ROOT_RESOURCE`](/ensv2/enhanced-access-control#resources) applies to every name in the registry. That reach is what makes some roles dangerous: `ROLE_SET_RESOLVER` on `ROOT_RESOURCE` can change the resolver of every name, `ROLE_SET_SUBREGISTRY` can swap any name's subregistry, `ROLE_UNREGISTER` can delete any name, and `ROLE_UPGRADE` can replace the registry implementation entirely. These four roles and their admin counterparts are the **dangerous roles**: `isEmancipated()` returns `true` once none of them has an assignee on `ROOT_RESOURCE`.

Operational roles are not dangerous and do not affect emancipation: `ROLE_REGISTRAR` can only register available names and `ROLE_RENEW` only extends expiry. This is how the `.eth` registry works: the [ETH Registrar](/ensv2/eth-registrar) holds both on `ROOT_RESOURCE`, while each name owner holds the roles defined by [`REGISTRATION_ROLE_BITMAP`](/ensv2/eth-registrar#roles-granted-at-registration). Emancipation is irreversible: re-granting a dangerous role requires its admin counterpart, which is itself a dangerous role that no account holds anymore.

To see which roles still block emancipation, inspect the per-role assignee counts with [`roleCount(ROOT_RESOURCE)`](/ensv2/enhanced-access-control#bitmap-layout). And rely on `isEmancipated()` only after [verifying](/ensv2/verifiable-factory#on-chain-verification) which implementation the registry runs: an arbitrary contract can simply always return `true`. The migration-specific `WrapperRegistry` hardcodes the flag to `true`, with its root roles derived at migration time from the name's [Name Wrapper fuses](/ensv2/migration).
...
<ContractReference functions={[
  ...
  {
    name: 'renew',
    description: 'Extend a name\'s expiry. Cannot reduce the current expiry. Requires ROLE_RENEW (root or name scope) while the name is unexpired; reviving an expired name requires ROLE_RENEW on ROOT_RESOURCE.',
    params: [
      { name: 'anyId', type: 'uint256', description: 'Labelhash, token ID, or resource of the name' },
  ...
  {
    name: 'unsafeTransfer',
    description: 'Transfer a token without the two safe-transfer checks (emancipation and sole assignee). The owner must hold ROLE_CAN_TRANSFER_ADMIN, and the caller must be the owner or an approved operator.',
    params: [
      { name: 'to', type: 'address', description: 'Receiver of the token' },
      { name: 'tokenId', type: 'uint256', description: 'Token to transfer' },
      { name: 'data', type: 'bytes', description: 'Forwarded to the receiver hook' },
    ],
  },
  {
    name: 'unsafeBatchTransfer',
    description: 'Batch variant of unsafeTransfer. All tokens must belong to the same owner.',
    params: [
      { name: 'to', type: 'address', description: 'Receiver of the tokens' },
      { name: 'tokenIds', type: 'uint256[]', description: 'Tokens to transfer' },
      { name: 'data', type: 'bytes', description: 'Forwarded to the receiver hook' },
    ],
  },
  {
...
<ContractReference functions={[
  ...
  {
    name: 'isEmancipated',
    description: 'True when no dangerous role has an assignee on ROOT_RESOURCE. See Emancipation.',
    returns: [
      { name: 'emancipated', type: 'bool', description: 'Whether the registry is emancipated' },
    ],
  },
  {
    name: 'getExpiry',
  ...
  {
    name: 'latestOwnerOf',
    description: 'Get the recorded owner of a token, ignoring expiry. Burned or stale token IDs return the zero address.',
    params: [
