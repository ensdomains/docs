...
Both Universal Resolver versions share a base contract that implements all resolution logic: forward resolution, reverse resolution, [CCIP-Read](/resolvers/ccip-read) gateway batching, and callback chaining. The only function the base leaves abstract is `findResolver()`, which each version overrides with its own registry traversal strategy:
...
Functions for inspecting the registry hierarchy itself live on the separate [UniversalHelper](#universalhelper) contract, keeping the Universal Resolver focused on resolution.
...
ENSv2 provides functions for locating registries within the hierarchy and verifying their position. All functions described here are exposed on the **UniversalHelper** contract, a companion to the Universal Resolver deployed alongside it (see [Deployments](/learn/deployments#sepolia-ensv2-beta)).
...
This is the registry that holds ownership information for a name, which is what [`findExactOwner`](#findexactowner-and-findnearestowner) queries.
...
Returns `address(0)` if the parent registry doesn't exist, doesn't implement `IOwnedRegistry`, or the label has no owner (unregistered, expired, or [reserved](/ensv2/permissioned-registry#name-lifecycle)).
...
This answers "who is responsible for this name?" for subnames that exist only as [resolver data](/ensv2/permissioned-resolver): a name with no registry entry of its own has no owner, but its closest registered ancestor does.
...
<ContractReference functions={[
  ...
  },
  {
    ...
    description: 'Like resolve, but first normalizes the name onchain via the supplied ENSIP-15 normalizer contract. Intended for future use, no normalizer contract is deployed yet. The variant resolveWithGatewaysAndNormalization additionally takes custom gateway URLs.',
    params: [
      { name: 'name', type: 'bytes', description: 'DNS-encoded name to resolve' },
      { name: 'data', type: 'bytes', description: 'ABI-encoded resolver calldata' },
      { name: 'ensip15', type: 'IENSIP15', description: 'Normalizer contract implementing normalize(label)' },
    ],
    returns: [
      { name: 'result', type: 'bytes', description: 'ABI-encoded response' },
      { name: 'resolver', type: 'address', description: 'The resolver that was used' },
  ...
  },
  {
    ...
    description: 'Like reverse, but verifies onchain that the returned primary name is normalized per the supplied ENSIP-15 normalizer contract. Intended for future use. The variant reverseWithGatewaysAndNormalization additionally takes custom gateway URLs.',
    params: [
      { name: 'lookupAddress', type: 'bytes', description: 'Byte-encoded address to look up' },
      { name: 'coinType', type: 'uint256', description: 'Coin type of the address' },
      { name: 'ensip15', type: 'IENSIP15', description: 'Normalizer contract implementing normalize(label)' },
  ...
  {
    name: 'normalize',
    description: 'Normalize a DNS-encoded name label by label via the supplied ENSIP-15 normalizer contract.',
    params: [
      { name: 'name', type: 'bytes', description: 'DNS-encoded name' },
      { name: 'ensip15', type: 'IENSIP15', description: 'Normalizer contract implementing normalize(label)' },
    ],
    returns: [
      { name: 'wasNormalized', type: 'bool', description: 'True if the input was already normalized' },
      { name: 'normalizedName', type: 'bytes', description: 'The normalized DNS-encoded name' },
    ],
  },
  {
    name: 'isENSv2',
    description: 'Marker distinguishing the v2 Universal Resolver from v1. Returns true.',
    returns: [
      { name: 'ensv2', type: 'bool', description: 'Always true on UniversalResolverV2' },
    ],
  },
]} />
...
<ContractReference functions={[
  {
    ...
    returns: [
      { name: 'owner', type: 'address', description: 'Owner address, or address(0) if unowned, not found, or the registry does not implement IOwnedRegistry' },
    ],
  },
  {
    name: 'findNearestOwner',
    description: 'Find the owner of the closest owned ancestor of a name.',
    params: [
      { name: 'name', type: 'bytes', description: 'DNS-encoded name' },
    ],
    returns: [
      { name: 'owner', type: 'address', description: 'Nearest owner, or address(0) if none found' },
      { name: 'offset', type: 'uint256', description: 'Byte offset into the name such that findExactOwner(name[offset:]) returns the same owner' },
    ],
  },
  {
  ...
  {
    name: 'findNearestRegistry',
    description: 'Find the deepest registry along the path of a name.',
    params: [
      { name: 'name', type: 'bytes', description: 'DNS-encoded name' },
    ],
    returns: [
      { name: 'registry', type: 'IRegistry', description: 'Nearest registry, or address(0) if not found' },
      { name: 'offset', type: 'uint256', description: 'Byte offset into the name such that findExactRegistry(name[offset:]) returns the same registry' },
    ],
  },
  {
...
<ContractReference functions={[
  {
    name: 'ROOT_REGISTRY',
    description: 'The ENSv2 root registry. All hierarchy traversal starts from this address. Set once at deployment (immutable). UniversalHelper exposes the same constant.',
  },
  {
...
<ContractReference functions={[
  ...
  },
  {
    ...
    description: 'resolveWithNormalization (or its gateway variant) was given a non-normalized name. The error carries the result for the normalized name instead.',
    params: [
      { name: 'normalizedName', type: 'bytes', description: 'The normalized DNS-encoded name' },
      { name: 'result', type: 'bytes', description: 'ABI-encoded response for the normalized name' },
      { name: 'resolver', type: 'address', description: 'The resolver that was used' },
    ],
  },
  {
    name: 'PrimaryNameNotNormalized',
    description: 'reverseWithNormalization (or its gateway variant) found a primary name that is not normalized.',
    params: [
      { name: 'primary', type: 'string', description: 'The non-normalized primary name' },
    ],
  },
  {
    name: 'HttpError',
