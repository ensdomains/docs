...
**Transfer griefing** is solved by `tokenVersionId`. When Alice tries to frontrun the sale by revoking roles on the listed name, the role change triggers a regeneration: the old token is burned and a new one with a new ID is minted. The marketplace's transfer still references the old token ID, so it cannot go through - the token it points to no longer exists. If the buyer still wants the name, the order must be re-created against the new token ID, at which point the roles can be inspected again.

Note that legitimate transfers move roles from the old owner to the new owner without triggering regeneration, so a transfer never invalidates the token ID it is using.

Regeneration is not the only safeguard: a safe transfer also makes sure the buyer actually receives what they expect. It reverts in two more situations:

- `TransferUnsafeUntilRegistryIsEmancipated`: the registry holding the name is not [emancipated](/ensv2/permissioned-registry#emancipation), so an account with root roles could still override the new owner.
- `TransferUnsafeWithMultipleAssignees`: an account other than the sender still holds roles on the token, so the name would arrive with third parties controlling parts of it.

Users who want to knowingly bypass these checks can use the registry's unsafe transfer variants, `unsafeTransfer` and `unsafeBatchTransfer`.
...
<FrenCallout fren="peanut" variant="note" title="Quick rule">
