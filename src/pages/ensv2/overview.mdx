...
<FrenCallout fren="bittu" variant="note" title="ETHOnline 2026">

A dedicated ENSv2 deployment is live on Sepolia for the ETHOnline hackathon.
Its addresses are listed under
[Deployments](/learn/deployments#sepolia-ensv2-beta): please build your
hackathon project against these, not the addresses found on the production
docs.

For resolution to work against the hackathon deployment, the Universal
Resolver address built into viem and ethers must be overwritten once in your
code with the one from the
[Deployments section](/learn/deployments#sepolia-ensv2-beta), where you can
find ready-made snippets.

These serve the hackathon deployment exclusively:

- [ENS Explorer](https://hackathon-deployment-portal-app.ens-cf.workers.dev/)
- [ENS App](https://hackathon-deployment-manager-app-v4.ens-cf.workers.dev/), where you can register a name in the hackathon ENSv2 deployment

Registration also works directly against the contracts:

1. Mint yourself test USDC (`MockUSDC` has an open `mint(to, amount)`)
2. `approve` the `ETHRegistrar` for the fee from `getRegisterPrice(label, duration, paymentToken)`
3. Call `commit(makeCommitment(label, owner, secret, subregistry, resolver, duration, referrer))`
4. Wait 60 seconds
5. Call `register` with the same parameters plus `MockUSDC` as the payment token

`subregistry` and `resolver` can be the zero address and set later. Both
contract addresses are in the
[Deployments table](/learn/deployments#sepolia-ensv2-beta).
